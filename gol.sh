...
n=$((echo 2o;seq -sp 225)|dc|xargs printf "%.8i\n"|tr 01 \ O);z='soOo[OS]og;si i[ +B]ig;ysoz' #    by Yownas
Q='BEGIN{FS=""}/^ *(O *){3}$/{print ";:L"++i";sw("$1$2$3"'$e'"$4")o("$5"'$e'"$6$7$8")w\\1z\\2w;tL"i}' #  :)
m=$(gawk "$(sed s1312,31<<<$Q)"<<<$n|sed "${z}sOSs";gawk "$Q"<<<$n|sed "${z}Ls Bls")";sy.-.y\nyg;ysOSBs OOs"
while o=$(sed -Ez "ys\ns-s;sy-y - yg$m"<<<"$E+-$o-$E+"|grep -v +);do clear;printf "$o\nGen: $(( ++i ))";done
